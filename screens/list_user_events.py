...
class AttendanceMySelectableRows(BoxLayout, RecycleDataViewBehavior, Button):
    ...
    def refresh_view_attrs(self, rv, index, data):
        ...
        return super(AttendanceMySelectableRows,
    ...
    def on_release(self):
        ...
        msgbox = AttendanceMessageBox(
            id_main=self.id_main,
...
class ListUserEventsScreen(Screen):
    ...
    def row_screen_dict(self, row: dict):
        return {
            'idx': row["index"],
            'disp': self.row_to_str(row),
            'a': str(row['id_event']),
            'b': str(row['id_client']),
            'c': bool(row['is_present']),
            'd': str(row['time_presence']),
            'e': ''
        }
    ...
    def add_rows_for_recycler_view(self, user_input=None):
        ...
        self.ids.my_list.data = [h] + [
            self.row_screen_dict(row) for row in rows
    ...
    def update_row(self, index, updated_row):
        for i in range(len(self.ids.my_list.data)):
            if self.ids.my_list.data[i]['idx'] == index:
                logging.info(
                    f"updating row in interface idx={index} row={updated_row}")
                self.ids.my_list.data[i] = self.row_screen_dict(updated_row)
                return
        log_exception(
            f"row with idx={index} not found",
            f"updating row in interface idx={index} row={updated_row}")
