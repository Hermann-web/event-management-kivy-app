...
def save_to_temp(collection_name, rows, temp_path):
    ...
    try:
        ...
        logging.error(
            f"error when trying to save in temp, collection {collection_name}. e:{e}"
        )
...
def fetch_all_data(collection_name, temp_path=None, filters=None):
    ...
    """
    if filters:
        logging.debug(f"setting uo filters")
        filters = {key: val for key, val in filters.items() if val}
    else:
        filters = None
    if not temp_path:
        logging.info(
            f"temp path will not be set for collection {collection_name}")
    ...
    #     logging.critical(f"temp_path {temp_path}: his folder does not exists and could not created. If It is a static table, it is better to save in temp in storage is not a problem")

    # if the temp file not exists, fetch online
    if not os.path.exists(temp_path):
        # fetch online
        logging.info(
            f"temp_path {temp_path} not found. fetching online collection {collection_name}"
        )
...
def get_clients(filters=None):
    ...
    """
    return list(
        fetch_all_data(collection_name=COLLECTION_CLIENTS,
                       temp_path=CLIENTS_TEMP_PATH,
                       filters=filters))
...
def get_events(filters=None):
    ...
    """
    return list(
        fetch_all_data(collection_name=COLLECTION_EVENTS,
                       temp_path=EVENTS_TEMP_PATH,
                       filters=filters))
...
def get_client_choices(filters=None):
    ...
    """
    return list(
        fetch_all_data(collection_name=COLLECTION_CLIENT_CHOICES,
                       temp_path=None,
                       filters=filters))
