...
def parse_mongo_obj_to_json_serializable(list_obj):
    list_obj = list(list_obj)  #because obj
    for c in list_obj:
        c['_id'] = str(c['_id'])
    return list(list_obj)  #can't return cursor obj
...
def catch_exceptions(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logging.info(
                f"function details: name={func_name} args={args} kwargs={kwargs}"
            )
            log_exception(
                err=e,
                action=
                f"wrapping {func_name} exceptions (RETURNING NONE): \nmodule:{func.__module__} \nannotations:{func.__annotations__} \ndocstring:{func.__doc__}"
            )
            result = None
        return result
